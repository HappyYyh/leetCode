 ...
 *
 * @ClassName: Simple_475
 * @description: 供暖器
 * @author: yyh
 * @create: 2019-12-21 10:45
 **/
public class Simple_475 {
    ...
    private static int findRadius(int[] houses, int[] heaters) {
        ...
        while (!containsAll){
            ...
            for (long heater : heaters) {
                for (long j = heater - r > 0 ? heater - r : 1; j <= heater + r; j++) {
                    set.add(j);
                }
            }
            int size = 0;
     ...
     * @return
     */
    private static int findRadius3(int[] houses, int[] heaters) {
        Arrays.sort(houses);
        Arrays.sort(heaters);
        int j = 0;
        int max = -1;
    ...
    public static void main(String[] args) {
        int[] houses = {1,2,3,4,5,6,10};
        int[] heaters = {1,4};
        System.out.println(findRadius3(houses,heaters));
    }
}
